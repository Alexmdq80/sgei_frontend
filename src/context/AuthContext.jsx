...
import catalogCache from "../services/catalogCacheService";
...
export const AuthProvider = ({ children }) => {
  ...
  /**
   * Verifica la autenticación inicial al cargar la aplicación.
   */
  const checkAuth = async () => {
    try {
      ...
      if (
        activeProfile &&
        activeProfile.type === "school" &&
        !data.user.escuelas_personas?.some(
          (link) =>
  ...
  // 1. Chequeo de sesión al montar
  useEffect(() => {
    checkAuth();
  }, []);

  // 2. Sincronización inteligente de catálogos en segundo plano al estar autenticado
  useEffect(() => {
    if (user) {
      const normalize = (r) => r?.data?.data || r?.data || r || [];

      const fetchers = {
        provincias: () => geografiaService.getProvincias(),
        regiones: () => geografiaService.getRegiones(),
        naciones: async () =>
          normalize(await nacionService.getAll({ per_page: 1000 })),
        documento_tipos: async () =>
          normalize(await documentoTipoService.getAll()),
        documento_situacions: async () =>
      ...
      catalogCache.syncWithManifest(fetchers);
    }
  }, [user]);
  ...
  const value = {
    ...
    checkAuth,
    hasPermission,
    clearCatalogCache: () => catalogCache.clearAll(),
  };
