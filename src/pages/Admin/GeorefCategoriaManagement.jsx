...
/**
 * Gestión de Categorías Georef.
 * Estándar Visual Gentleman: Alta fidelidad y UX profesional.
 */
const GeorefCategoriaManagement = () => {
    ...
    const fetchItems = useCallback(async () => {
        try {
            setIsLoading(true);
            const response = await georefCategoriaService.getAll({ 
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingItem) {
                await georefCategoriaService.update(editingItem.id, formData);
                showNotification('Categoría actualizada con éxito.', 'success');
            } else {
                await georefCategoriaService.create(formData);
    ...
    const handleDelete = (item) => {
        setConfirmConfig({
            ...
            message: `¿Estás seguro de que deseas eliminar la categoría "${item.nombre}"? Esta acción no se puede deshacer.`,
            variant: 'danger',
            onConfirm: async () => {
                try {
                    await georefCategoriaService.delete(item.id);
    ...
    return (
        <div className="space-y-8 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-secondary-900 tracking-tighter">Categorías Georef</h1>
                    <p className="text-secondary-500 mt-2 font-medium">Clasificación taxonómica de entidades geográficas</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-secondary-200 overflow-hidden">
                {isLoading ? (
                    <div className="p-24 flex flex-col items-center justify-center">
                        ...
                        <p className="text-secondary-400 font-black uppercase text-[10px] tracking-[0.2em]">Sincronizando categorías...</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-secondary-50/50 border-b border-secondary-100">
                                <tr>
                                    <th className="px-8 py-5 text-[10px] font-black text-secondary-500 uppercase tracking-widest">Nombre de Categoría</th>
                                ...
                                {items.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan="3" className="px-8 py-20 text-center">
                                            <p className="text-secondary-400 font-bold italic">No se encontraron categorías.</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn border border-white/20">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="p-10 space-y-6">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-500 uppercase tracking-[0.15em] mb-2">Nombre de la Categoría</label>
                                    <input 
                                        type="text" required
                                ...
                                <div className="flex items-center justify-between p-5 bg-secondary-50 rounded-2xl border border-secondary-200 shadow-inner">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary-500 font-medium mt-1">Habilitar esta categoría en el sistema</p>
                                    </div>
                            ...
                            <div className="p-8 bg-secondary-50/50 border-t border-secondary-100 flex gap-4">
                                ...
                                <button type="submit" className="flex-[2] py-4 bg-primary-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-primary-700 shadow-lg shadow-primary-200 transition-all active:scale-[0.98]">
                                    {editingItem ? 'Sincronizar Cambios' : 'Crear Categoría'}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default GeorefCategoriaManagement;
