...
const TurnoManagement = () => {
    ...
    const fetchItems = async () => {
        try {
            ...
            showNotification(parseError(error, 'Error al cargar los turnos.'), 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsSaving(true);
            if (editingItem) {
                await turnoService.update(editingItem.id, formData);
    ...
    const handleDelete = (item) => {
        setConfirmConfig({
            ...
            message: `¿Está seguro que desea eliminar el turno "${item.nombre}"? Esta acción no se puede deshacer.`,
            onConfirm: async () => {
                try {
                    await turnoService.delete(item.id);
                    showNotification('Turno eliminado correctamente.', 'success');
                    fetchItems();
                } catch (error) {
                    showNotification(parseError(error, 'Error al eliminar el turno.'), 'error');
                } finally {
        ...
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-secondary-500 text-sm mt-1">Administre los turnos escolares (Mañana, Tarde, Noche, etc.).</p>
                </div>
                <button 
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-secondary-100">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan="3" className="px-6 py-12 text-center text-secondary-500 font-medium">No se encontraron turnos.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-secondary-900/60 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-md overflow-hidden animate-scaleIn">
                        <div className="px-8 py-6 border-b border-secondary-100 flex items-center justify-between bg-secondary-50/50">
                            <div>
                                <h3 className="text-xl font-bold text-secondary-900">{editingItem ? 'Editar Turno' : 'Nuevo Turno'}</h3>
                                <p className="text-secondary-500 text-xs mt-1">Complete los datos del turno.</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-500 uppercase tracking-widest mb-1.5">Nombre del Turno</label>
                                    <input type="text" required value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} className="w-full px-4 py-3 bg-secondary-50 border border-secondary-200 rounded-2xl text-sm" placeholder="Ej: TURNO MAÑANA" />
                                </div>
                                <div>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button type="submit" disabled={isSaving} className="flex-1 px-6 py-3 bg-primary-600 text-white rounded-2xl font-bold hover:bg-primary-700 shadow-lg shadow-primary-200 disabled:opacity-50">
                                    {isSaving ? 'Guardando...' : (editingItem ? 'Guardar Cambios' : 'Crear Turno')}
                                </button>
                            </div>
                        </form>
                    </div>
