...
/**
 * Gestión de Ofertas Educativas del Sistema.
 * Ubicado en Panel General > Ofertas Educativas.
 */
const OfertaManagement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingOferta) {
                await ofertaService.update(editingOferta.id, formData);
                showNotification('Oferta actualizada con éxito.', 'success');
            } else {
                await ofertaService.create(formData);
                showNotification('Oferta creada con éxito.', 'success');
            }
    ...
    const handleDelete = (oferta) => {
        setConfirmConfig({
            ...
            message: `¿Estás seguro de que deseas eliminar la oferta "${oferta.nombre}"? Esta acción no se puede deshacer.`,
            variant: 'danger',
            onConfirm: async () => {
                try {
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Otras Ofertas Educativas</h1>
                    <p className="text-secondary-500 mt-1 font-medium">Gestión de la oferta académica complementaria</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-secondary-200 overflow-hidden max-w-4xl">
                {isLoading ? (
                    <div className="p-20 flex flex-col items-center justify-center">
                        ...
                        <p className="text-secondary-500 font-bold">Cargando ofertas...</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-secondary-50 border-b border-secondary-200">
                                <tr>
                                    <th className="px-6 py-4 text-[10px] font-black text-secondary-400 uppercase tracking-widest">Nombre de la Oferta</th>
                            ...
                            <tbody className="divide-y divide-secondary-100">
                                {ofertas.map((oferta) => (
                                    <tr key={oferta.id} className="hover:bg-secondary-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-secondary-900 tracking-tight uppercase">{oferta.nombre}</p>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 text-[10px] font-black rounded-lg uppercase ${
                                                oferta.vigente ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                            }`}>
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="p-8 space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-400 uppercase mb-1">Nombre de la Oferta</label>
                                    <input 
                                        type="text" required
                                        ...
                                        placeholder="Ej: BACHILLERATO EN CIENCIAS SOCIALES"
                                ...
                                <div className="flex items-center justify-between p-4 bg-secondary-50 rounded-2xl border border-secondary-200">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary-500 font-medium">Habilitar esta oferta para las instituciones</p>
                                    </div>
...
export default OfertaManagement;
