...
import vinculoTipoService from '../../services/vinculoTipoService';
import ConfirmationModal from '../../components/ConfirmationModal';

const VinculoManagement = () => {
    const { showNotification } = useAuth();
    const [items, setItems] = useState([]);
    const [vinculoTipos, setVinculoTipos] = useState([]);
    ...
    const fetchItems = async () => {
        try {
            setIsLoading(true);
            const data = await vinculoService.getAll();
            setItems(Array.isArray(data) ? data : []);
        } catch (error) {
            showNotification(parseError(error, 'Error al cargar los vínculos.'), 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const fetchCatalogs = async () => {
        try {
            const data = await vinculoTipoService.getAll();
            setVinculoTipos(Array.isArray(data) ? data : []);
        } catch (error) {
    ...
    const handleOpenModal = (item = null) => {
        if (item) {
            ...
            setFormData({
                nombre: item.nombre,
                vinculo_tipo_id: item.vinculo_tipo_id || '',
                orden: item.orden || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsSaving(true);
            if (editingItem) {
                await vinculoService.update(editingItem.id, formData);
                showNotification('Vínculo actualizado correctamente.', 'success');
            } else {
                await vinculoService.create(formData);
                showNotification('Vínculo creado correctamente.', 'success');
            }
            fetchItems();
            handleCloseModal();
        } catch (error) {
    ...
    const handleDelete = (item) => {
        setConfirmConfig({
            ...
            onConfirm: async () => {
                try {
                    await vinculoService.delete(item.id);
    ...
    const filteredItems = items.filter(item => 
        ...
        item.vinculo_tipo?.nombre?.toLowerCase().includes(search.toLowerCase())
    );
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-secondary-900">Vínculos</h1>
                    <p className="text-secondary-500 text-sm mt-1">Gestione los tipos específicos de relación (ej: PADRE, MADRE, TUTOR).</p>
                </div>
                <button 
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-secondary-50/50">
                                ...
                                <th className="px-6 py-4 text-[10px] font-black text-secondary-500 uppercase tracking-widest border-b border-secondary-100">Tipo General</th>
                        ...
                        <tbody className="divide-y divide-secondary-100">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan="4" className="px-6 py-12 text-center text-secondary-500 font-medium">No se encontraron vínculos.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-secondary-900/60 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn">
                        <div className="px-8 py-6 border-b border-secondary-100 flex items-center justify-between bg-secondary-50/50">
                            <div>
                                <h3 className="text-xl font-bold text-secondary-900">{editingItem ? 'Editar Vínculo' : 'Nuevo Vínculo'}</h3>
                                <p className="text-secondary-500 text-xs mt-1">Complete los datos de la relación.</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-500 uppercase tracking-widest mb-1.5">Tipo General</label>
                                    <select 
                                        required
                                        value={formData.vinculo_tipo_id}
                                        onChange={(e) => setFormData({...formData, vinculo_tipo_id: e.target.value})}
                                        className="w-full px-4 py-3 bg-secondary-50 border border-secondary-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all appearance-none"
                                    >
                                        <option value="">Seleccione un tipo...</option>
                                        {vinculoTipos.map(type => (
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-500 uppercase tracking-widest mb-1.5">Nombre del Vínculo</label>
                                    <input type="text" required value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} className="w-full px-4 py-3 bg-secondary-50 border border-secondary-200 rounded-2xl text-sm" placeholder="Ej: PADRE, MADRE, TUTOR LEGAL" />
                                </div>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button type="submit" disabled={isSaving} className="flex-1 px-6 py-3 bg-primary-600 text-white rounded-2xl font-bold hover:bg-primary-700 shadow-lg shadow-primary-200 disabled:opacity-50">
                                    {isSaving ? 'Guardando...' : (editingItem ? 'Guardar Cambios' : 'Crear Vínculo')}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default VinculoManagement;
