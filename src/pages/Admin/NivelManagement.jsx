...
/**
 * Gestión de Niveles Educativos del Sistema.
 * Ubicado en Panel General > Niveles.
 */
const NivelManagement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingNivel) {
                await nivelService.update(editingNivel.id, formData);
                showNotification('Nivel actualizado con éxito.', 'success');
            } else {
                await nivelService.create(formData);
    ...
    const handleDelete = (nivel) => {
        setConfirmConfig({
            ...
            onConfirm: async () => {
                try {
                    await nivelService.delete(nivel.id);
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Niveles Educativos</h1>
                    <p className="text-secondary-500 mt-1 font-medium">Gestión de niveles de enseñanza (Primaria, Secundaria, etc.)</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-secondary-200 overflow-hidden max-w-4xl">
                {isLoading ? (
                    <div className="p-20 flex flex-col items-center justify-center">
                        ...
                        <p className="text-secondary-500 font-bold">Cargando niveles...</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-secondary-100">
                                {niveles.map((nivel) => (
                                    <tr key={nivel.id} className="hover:bg-secondary-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-secondary-900 tracking-tight uppercase">{nivel.nombre}</p>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 text-[10px] font-black rounded-lg uppercase ${
                                                nivel.vigente ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                            }`}>
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="p-8 space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-400 uppercase mb-1">Nombre del Nivel</label>
                                    <input 
                                        type="text" required
                                ...
                                <div className="flex items-center justify-between p-4 bg-secondary-50 rounded-2xl border border-secondary-200">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary-500 font-medium">Habilitar este nivel para las escuelas</p>
                                    </div>
...
export default NivelManagement;
