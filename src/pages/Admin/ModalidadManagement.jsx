...
/**
 * Gestión de Modalidades Educativas del Sistema.
 * Ubicado en Panel General > Modalidades.
 */
const ModalidadManagement = () => {
    ...
    const handleOpenEdit = (modalidad) => {
        setEditingModalidad(modalidad);
        setFormData({
            nombre: modalidad.nombre,
            vigente: !!modalidad.vigente
        });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingModalidad) {
                await modalidadService.update(editingModalidad.id, formData);
                showNotification('Modalidad actualizada con éxito.', 'success');
            } else {
                await modalidadService.create(formData);
                showNotification('Modalidad creada con éxito.', 'success');
            }
    ...
    const handleDelete = (modalidad) => {
        setConfirmConfig({
            ...
            message: `¿Estás seguro de que deseas eliminar la modalidad "${modalidad.nombre}"? Esta acción no se puede deshacer.`,
            variant: 'danger',
            onConfirm: async () => {
                try {
                    await modalidadService.delete(modalidad.id);
                    showNotification('Modalidad eliminada con éxito.', 'success');
                    fetchModalidades();
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Modalidades Educativas</h1>
                    <p className="text-secondary-500 mt-1 font-medium">Gestión de modalidades (Técnica, Agraria, Especial, etc.)</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-secondary-200 overflow-hidden max-w-4xl">
                {isLoading ? (
                    <div className="p-20 flex flex-col items-center justify-center">
                        ...
                        <p className="text-secondary-500 font-bold">Cargando modalidades...</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-secondary-100">
                                {modalidades.map((modalidad) => (
                                    <tr key={modalidad.id} className="hover:bg-secondary-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-secondary-900 tracking-tight uppercase">{modalidad.nombre}</p>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 text-[10px] font-black rounded-lg uppercase ${
                                                modalidad.vigente ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                            }`}>
                                                {modalidad.vigente ? 'Vigente' : 'No Vigente'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button 
                                                    onClick={() => handleOpenEdit(modalidad)}
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="p-8 space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-400 uppercase mb-1">Nombre de la Modalidad</label>
                                    <input 
                                        type="text" required
                                ...
                                <div className="flex items-center justify-between p-4 bg-secondary-50 rounded-2xl border border-secondary-200">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary-500 font-medium">Habilitar esta modalidad para las escuelas</p>
                                    </div>
...
export default ModalidadManagement;
