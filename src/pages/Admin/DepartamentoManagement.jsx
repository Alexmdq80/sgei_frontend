...
const DepartamentoManagement = () => {
    ...
    useEffect(() => {
        if (selectedNacionId) {
            const filtered = provincias.filter(p => p.nacion_id == selectedNacionId);
            setFilteredProvincias(filtered);
            // Si la provincia actual no pertenece a la nueva nación, resetearla
            const currentProv = provincias.find(p => p.id == formData.provincia_id);
            if (currentProv && currentProv.nacion_id != selectedNacionId) {
                setFormData(prev => ({ ...prev, provincia_id: '' }));
            }
        } else {
    ...
    const filteredItems = items.filter(item => 
        ...
        item.provincia?.nombre.toLowerCase().includes(search.toLowerCase()) ||
        item.provincia?.nacion?.nombre.toLowerCase().includes(search.toLowerCase())
    );
                            ...
                            {isLoading ? (
                                ...
                                filteredItems.map((item) => (
                                    <tr key={item.id} className="hover:bg-secondary-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-sm text-secondary-600">{item.provincia?.nombre}</td>
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-12 text-center">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p className="text-secondary-500 font-medium">No se encontraron departamentos.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
