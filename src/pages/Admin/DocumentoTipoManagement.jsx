...
const DocumentoTipoManagement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsSaving(true);
            if (editingItem) {
                await documentoTipoService.update(editingItem.id, formData);
                showNotification('Tipo de documento actualizado correctamente.', 'success');
            } else {
                await documentoTipoService.create(formData);
    ...
    const handleDelete = (id) => {
        setConfirmConfig({
            ...
            onConfirm: async () => {
                try {
                    await documentoTipoService.delete(id);
    ...
    return (
        <div className="container mx-auto px-4 py-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-secondary-900">Tipos de Documento</h1>
                    <p className="text-sm font-medium text-secondary-500">Administra los tipos de identificación permitidos en el sistema.</p>
                </div>
                <button
                                ...
                                <div className="space-y-5">
                                    <div>
                                        <label className="block text-[10px] font-black text-secondary-400 uppercase mb-1 ml-1">
                                            Nombre del Tipo (Ej: DNI, Pasaporte)
                                        </label>
                                        <input
                                            type="text"
                                            required
...
export default DocumentoTipoManagement;
