...
const PuestoTipoManagement = () => {
    ...
    const fetchItems = async () => {
        try {
            setIsLoading(true);
            const data = await puestoTipoService.getAll();
            setItems(Array.isArray(data) ? data : []);
        } catch (error) {
            showNotification(parseError(error, 'Error al cargar los tipos de puesto.'), 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsSaving(true);
            if (editingItem) {
                await puestoTipoService.update(editingItem.id, formData);
                showNotification('Tipo de puesto actualizado correctamente.', 'success');
            } else {
                await puestoTipoService.create(formData);
                showNotification('Tipo de puesto creado correctamente.', 'success');
            }
            fetchItems();
            handleCloseModal();
        } catch (error) {
    ...
    const handleDelete = (item) => {
        setConfirmConfig({
            ...
            onConfirm: async () => {
                try {
                    await puestoTipoService.delete(item.id);
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-secondary-900">Tipos de Puesto</h1>
                    <p className="text-secondary-500 text-sm mt-1">Gestione los tipos de puesto (Cargo, Horas, etc.).</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-secondary-200 overflow-hidden">
                <div className="p-4 border-b border-secondary-100 bg-secondary-50/50">
                    <div className="relative max-w-md">
                        ...
                        <input 
                            type="text" 
                            placeholder="Buscar tipo de puesto..." 
                            value={search}
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-secondary-100">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan="3" className="px-6 py-12 text-center text-secondary-500 font-medium">No se encontraron tipos de puesto.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn">
                        <div className="px-8 py-6 border-b border-secondary-100 flex items-center justify-between bg-secondary-50/50">
                            <div>
                                ...
                                <p className="text-secondary-500 text-xs mt-1">Defina el nombre del tipo de puesto.</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" required value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} className="w-full px-4 py-3 bg-secondary-50 border border-secondary-200 rounded-2xl text-sm" placeholder="Ej: CARGO, MÓDULOS, etc." />
                                </div>
...
export default PuestoTipoManagement;
