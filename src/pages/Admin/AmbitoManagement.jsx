...
/**
 * Gestión de Ámbitos Escolares del Sistema.
 * Ubicado en Panel General > Ámbitos.
 */
const AmbitoManagement = () => {
    ...
    const handleOpenCreate = () => {
        setEditingAmbito(null);
        setFormData({ nombre: '', vigente: true });
        setIsModalOpen(true);
    };

    const handleOpenEdit = (ambito) => {
        setEditingAmbito(ambito);
        setFormData({
            nombre: ambito.nombre,
            vigente: !!ambito.vigente
        });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingAmbito) {
                await ambitoService.update(editingAmbito.id, formData);
                showNotification('Ámbito actualizado con éxito.', 'success');
            } else {
                await ambitoService.create(formData);
                showNotification('Ámbito creado con éxito.', 'success');
            }
    ...
    const handleDelete = (ambito) => {
        setConfirmConfig({
            isOpen: true,
            title: 'Eliminar Ámbito',
            message: `¿Estás seguro de que deseas eliminar el ámbito "${ambito.nombre}"? Esta acción no se puede deshacer.`,
            variant: 'danger',
            onConfirm: async () => {
                try {
                    await ambitoService.delete(ambito.id);
                    showNotification('Ámbito eliminado con éxito.', 'success');
                    fetchAmbitos();
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Ámbitos Escolares</h1>
                    <p className="text-secondary-500 mt-1 font-medium">Gestión de entornos geográficos y sociales de las escuelas</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-secondary-200 overflow-hidden max-w-4xl">
                {isLoading ? (
                    <div className="p-20 flex flex-col items-center justify-center">
                        ...
                        <p className="text-secondary-500 font-bold">Cargando ámbitos...</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-secondary-100">
                                {ambitos.map((ambito) => (
                                    <tr key={ambito.id} className="hover:bg-secondary-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-secondary-900 tracking-tight uppercase">{ambito.nombre}</p>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 text-[10px] font-black rounded-lg uppercase ${
                                                ambito.vigente ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                            }`}>
                                                {ambito.vigente ? 'Vigente' : 'No Vigente'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button 
                                                    onClick={() => handleOpenEdit(ambito)}
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="p-6 border-b border-secondary-100 flex items-center justify-between bg-secondary-50">
                                <h2 className="text-xl font-black text-secondary-900">
                                    {editingAmbito ? 'Editar Ámbito' : 'Nuevo Ámbito'}
                                </h2>
                            ...
                            <div className="p-8 space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-400 uppercase mb-1">Nombre del Ámbito</label>
                                    <input 
                                        type="text" required
                                ...
                                <div className="flex items-center justify-between p-4 bg-secondary-50 rounded-2xl border border-secondary-200">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary-500 font-medium">Habilitar este ámbito para las escuelas</p>
                                    </div>
                            ...
                            <div className="p-6 bg-secondary-50 border-t border-secondary-100 flex gap-3">
                                ...
                                <button type="submit" className="flex-[2] py-3.5 bg-primary-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-primary-700 shadow-lg transition-all active:scale-[0.98]">
                                    {editingAmbito ? 'Guardar Cambios' : 'Crear Ámbito'}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default AmbitoManagement;
