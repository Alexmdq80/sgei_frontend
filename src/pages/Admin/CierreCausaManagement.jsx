...
/**
 * Gestión de Causas de Cierre del Sistema.
 * Ubicado en Panel General > Causas de Cierre.
 */
const CierreCausaManagement = () => {
    ...
    const fetchCausas = async () => {
        try {
            setIsLoading(true);
            const data = await cierreCausaService.getAll();
            setCausas(data);
        } catch (error) {
            showNotification('Error al cargar las causas de cierre.', 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleOpenEdit = (causa) => {
        setEditingCausa(causa);
        setFormData({
            nombre: causa.nombre,
            vigente: !!causa.vigente
        });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingCausa) {
                await cierreCausaService.update(editingCausa.id, formData);
                showNotification('Causa de cierre actualizada con éxito.', 'success');
            } else {
                await cierreCausaService.create(formData);
                showNotification('Causa de cierre creada con éxito.', 'success');
            }
    ...
    const handleDelete = (causa) => {
        setConfirmConfig({
            ...
            message: `¿Estás seguro de que deseas eliminar la causa "${causa.nombre}"? Esta acción no se puede deshacer.`,
            variant: 'danger',
            onConfirm: async () => {
                try {
                    await cierreCausaService.delete(causa.id);
                    showNotification('Causa eliminada con éxito.', 'success');
                    fetchCausas();
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Causas de Cierre</h1>
                    <p className="text-secondary-500 mt-1 font-medium">Gestión de motivos de cierre de inscripciones de estudiantes</p>
                </div>
                <button 
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-secondary-50 border-b border-secondary-200">
                                <tr>
                                    <th className="px-6 py-4 text-[10px] font-black text-secondary-400 uppercase tracking-widest">Nombre del Motivo</th>
                            ...
                            <tbody className="divide-y divide-secondary-100">
                                {causas.map((causa) => (
                                    <tr key={causa.id} className="hover:bg-secondary-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-secondary-900 tracking-tight uppercase">{causa.nombre}</p>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 text-[10px] font-black rounded-lg uppercase ${
                                                causa.vigente ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                            }`}>
                                                {causa.vigente ? 'Vigente' : 'No Vigente'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button 
                                                    onClick={() => handleOpenEdit(causa)}
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="p-8 space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-400 uppercase mb-1">Nombre de la Causa</label>
                                    <input 
                                        type="text" required
                                        ...
                                        placeholder="Ej: BAJA SOLICITADA POR EL ESTUDIANTE"
                                ...
                                <div className="flex items-center justify-between p-4 bg-secondary-50 rounded-2xl border border-secondary-200">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary-500 font-medium">Habilitar esta causa para el cierre de inscripciones</p>
                                    </div>
                            ...
                            <div className="p-6 bg-secondary-50 border-t border-secondary-100 flex gap-3">
                                ...
                                <button type="submit" className="flex-[2] py-3.5 bg-primary-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-primary-700 shadow-lg transition-all active:scale-[0.98]">
                                    {editingCausa ? 'Guardar Cambios' : 'Crear Causa'}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default CierreCausaManagement;
