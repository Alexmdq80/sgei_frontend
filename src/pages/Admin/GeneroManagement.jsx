...
const GeneroManagement = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            showNotification(parseError(error, 'Error al cargar los géneros.'), 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleOpenModal = (item = null) => {
        if (item) {
            ...
            setEditingItem(null);
            setFormData({ nombre: '', orden: 100, vigente: true });
        }
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsSaving(true);
            if (editingItem) {
                await generoService.update(editingItem.id, formData);
                showNotification('Género actualizado correctamente.', 'success');
            } else {
                await generoService.create(formData);
                showNotification('Género creado correctamente.', 'success');
            }
            fetchData();
    ...
    return (
        <div className="container mx-auto px-4 py-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-secondary-900">Gestión de Géneros</h1>
                    <p className="text-sm font-medium text-secondary-500">Administra las identidades de género registradas en el sistema.</p>
                </div>
                <button
                            ...
                            <div className="px-8 pt-8 pb-6">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-xl font-bold text-secondary-900">
                                        {editingItem ? 'Editar Género' : 'Nuevo Género'}
                                    </h3>
                                    <button 
                                        type="button"
                                ...
                                <div className="space-y-5">
                                    <div className="flex gap-4">
                                        <div className="flex-1">
                                            ...
                                            <input
                                                type="text"
                                                required
                                                className="w-full px-4 py-3 bg-secondary-50 border border-secondary-200 rounded-xl font-bold text-secondary-900 focus:ring-2 focus:ring-primary-500 outline-none transition-all placeholder:text-secondary-300"
                                        ...
                                        <div className="flex-none">
                                            ...
                                            <input
                                                ...
                                                min="0"
                                                max="255"
                                                className="w-24 px-4 py-3 bg-secondary-50 border border-secondary-200 rounded-xl font-bold text-secondary-900 focus:ring-2 focus:ring-primary-500 outline-none transition-all"
                                    ...
                                    <div className="flex items-center gap-3 p-4 bg-secondary-50 border border-secondary-200 rounded-xl">
                                        ...
                                        <label htmlFor="vigente-toggle-gen" className="text-sm font-bold text-secondary-700 cursor-pointer select-none">
                                            Género Vigente
                                        </label>
                                    </div>
                                </div>
...
export default GeneroManagement;
