...
/**
 * Gestión de Ubicaciones de Escuela (Ubicación Geográfica) del Sistema.
 * Ubicado en Panel General > Ubicaciones de Escuela.
 */
const EscuelaUbicacionManagement = () => {
    const { showNotification } = useAuth();
    const [escuelaUbicaciones, setEscuelaUbicaciones] = useState([]);
    const [isLoading, setIsLoading] = useState(true);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingEscuelaUbicacion, setEditingEscuelaUbicacion] = useState(null);
    ...
    const fetchEscuelaUbicaciones = async () => {
        try {
            setIsLoading(true);
    ...
    const handleOpenCreate = () => {
        setEditingEscuelaUbicacion(null);
        setFormData({ nombre: '', vigente: true });
        setIsModalOpen(true);
    };

    const handleOpenEdit = (ubicacion) => {
        setEditingEscuelaUbicacion(ubicacion);
        setFormData({
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if (editingEscuelaUbicacion) {
                await escuelaUbicacionService.update(editingEscuelaUbicacion.id, formData);
                showNotification('Ubicación de escuela actualizada con éxito.', 'success');
            } else {
                await escuelaUbicacionService.create(formData);
                showNotification('Ubicación de escuela creada con éxito.', 'success');
            }
    ...
    const handleDelete = (ubicacion) => {
        setConfirmConfig({
            isOpen: true,
            title: 'Eliminar Ubicación de Escuela',
            message: `¿Estás seguro de que deseas eliminar la ubicación de escuela "${ubicacion.nombre}"? Esta acción no se puede deshacer.`,
            variant: 'danger',
            onConfirm: async () => {
                try {
                    await escuelaUbicacionService.delete(ubicacion.id);
                    showNotification('Ubicación de escuela eliminada con éxito.', 'success');
                    fetchEscuelaUbicaciones();
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Ubicaciones de Escuela</h1>
                    <p className="text-secondary-500 mt-1 font-medium">Gestión de ubicaciones relativas de la escuela</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-secondary-200 overflow-hidden max-w-4xl">
                {isLoading ? (
                    <div className="p-20 flex flex-col items-center justify-center">
                        ...
                        <p className="text-secondary-500 font-bold">Cargando ubicaciones...</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-secondary-100">
                                {escuelaUbicaciones.map((ubicacion) => (
                                    <tr key={ubicacion.id} className="hover:bg-secondary-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-secondary-900 tracking-tight uppercase">{ubicacion.nombre}</p>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 text-[10px] font-black rounded-lg uppercase ${
                                                ubicacion.vigente ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                            }`}>
                                                {ubicacion.vigente ? 'Vigente' : 'No Vigente'}
                                            </span>
                                        </td>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="p-6 border-b border-secondary-100 flex items-center justify-between bg-secondary-50">
                                <h2 className="text-xl font-black text-secondary-900">
                                    {editingEscuelaUbicacion ? 'Editar Ubicación' : 'Nueva Ubicación'}
                                </h2>
                            ...
                            <div className="p-8 space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-secondary-400 uppercase mb-1">Nombre de la Ubicación</label>
                                    <input 
                                        type="text" required
                                        ...
                                        placeholder="Ej: MISMO BARRIO, OTRO BARRIO, ETC."
                                ...
                                <div className="flex items-center justify-between p-4 bg-secondary-50 rounded-2xl border border-secondary-200">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-secondary-500 font-medium">Habilitar esta ubicación para las escuelas</p>
                                    </div>
                            ...
                            <div className="p-6 bg-secondary-50 border-t border-secondary-100 flex gap-3">
                                ...
                                <button type="submit" className="flex-[2] py-3.5 bg-primary-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-primary-700 shadow-lg transition-all active:scale-[0.98]">
                                    {editingEscuelaUbicacion ? 'Guardar Cambios' : 'Crear Ubicación'}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default EscuelaUbicacionManagement;
