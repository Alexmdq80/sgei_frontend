...
const NacionManagement = () => {
    ...
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const [nacionesData, continentesData] = await Promise.all([
                nacionService.getAll(),
                continenteService.getAll()
            ]);
            setItems(nacionesData);
    ...
    const filteredItems = items.filter(item => 
        ...
        item.nacionalidad.toLowerCase().includes(search.toLowerCase()) ||
        item.continente?.nombre.toLowerCase().includes(search.toLowerCase())
    );
                            ...
                            {isLoading ? (
                                ...
                                filteredItems.map((item) => (
                                    <tr key={item.id} className="hover:bg-secondary-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-sm text-secondary-600">{item.nacionalidad}</td>
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-12 text-center">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p className="text-secondary-500 font-medium">No se encontraron naciones.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
