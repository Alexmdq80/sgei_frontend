...
import logoAjal from "../assets/logo.svg";
...
const Login = () => {
    ...
    const fetchDocumentoTipos = async () => {
      ...
      try {
        ...
        if (data.length > 0) {
          setCredentials((prev) => ({
            ...prev,
            documento_tipo_id: data[0].id,
          }));
        }
      } catch (err) {
        console.error("Error cargando tipos de documento:", err);
        setError(
          "No se pudieron cargar los tipos de documento. Intente recargar la página.",
        );
      } finally {
  ...
  const handleResendVerification = async () => {
    if (!credentials.email) {
      setError(
        "Por favor, ingrese su correo electrónico para reenviar la verificación.",
      );
      return;
    }
    ...
    try {
      ...
      if (err.response?.status === 429) {
        setError(
          "Demasiados intentos. Por favor, espera unos minutos antes de solicitar otro enlace.",
        );
      } else {
        setError(
          err.response?.data?.error ||
            err.response?.data?.message ||
            "Error al reenviar la verificación. Intente más tarde.",
        );
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (errorData?.errors?.email_unverified) {
        setError(
          "Tu cuenta no está verificada. Revisa tu correo o solicita un nuevo enlace.",
        );
      } else {
        setError(
          errorData?.error ||
    ...
    <div className="min-h-screen flex items-center justify-center bg-secondary-100 px-4 font-sans">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg border border-secondary-200 p-8 md:p-10 transition-all">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {loginMethod === "email" ? (
            <div>
              <label className="block text-xs font-bold text-secondary-500 uppercase tracking-wider mb-1 ml-1">
                Correo Electrónico
              </label>
          ...
          <div>
            ...
            <div className="flex flex-col items-end gap-1 mt-1">
              ...
              <Link
                to="/solicitar-activacion"
                className="text-[11px] font-bold text-primary-600 hover:text-primary-700 transition-colors uppercase tracking-wider"
              >
                ¿Fuiste invitado? Activa tu cuenta
              </Link>
            </div>
          </div>
