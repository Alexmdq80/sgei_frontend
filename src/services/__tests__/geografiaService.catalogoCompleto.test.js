...
async function servicioFresco() {
  ...
  return {
    api: apiMod.default,
    cache: cacheMod.default,
    geo: geoMod.default,
    normalizeSearch: geoMod.normalizeSearch,
  };
}
...
describe("geografiaService - catalogo completo de localidades", () => {
  ...
  it("syncCatalogoLocalidadesCompleto descarga y persiste (sin preindexar)", async () => {
  ...
  it("getCatalogoLocalidades devuelve la RAM sin tocar la red", async () => {
    ...
    expect(catalogo[0].nombre).toBe(CORDOBA);
    // Regresión del Punto 2: ya NO se preindexan claves en cada hidratación
    expect(catalogo[0]).not.toHaveProperty("_searchKey");
    expect(catalogo[0]).not.toHaveProperty("_nombreKey");
  });
  ...
  it("searchLocalidades NO consulta la API cuando el índice devuelve 0 coincidencias", async () => {
    const { geo, api, cache } = await servicioFresco();
    cache.getCatalogoLocalidades.mockResolvedValue(LOCS);

    const res = await geo.searchLocalidades("zzzzzz", 10);

    expect(res).toEqual([]);
    expect(api.get).not.toHaveBeenCalled(); // índice autoritativo
  });

  it("searchLocalidades tolera typos por transposición (fuzzy absoluto, no 0.2)", async () => {
    const { geo, cache } = await servicioFresco();
    cache.getCatalogoLocalidades.mockResolvedValue(LOCS);

    const res = await geo.searchLocalidades("tandli", 10);

    expect(res.map((l) => l.nombre)).toContain("TANDIL");
  });

  it("searchLocalidades ignora tildes y matchea por prefijo", async () => {
    const { geo, cache } = await servicioFresco();
    cache.getCatalogoLocalidades.mockResolvedValue(LOCS);

    const res = await geo.searchLocalidades("cord", 10);

    expect(res.map((l) => l.nombre)).toContain(CORDOBA);
    expect(res.map((l) => l.nombre)).not.toContain("TANDIL");
  });

  it("prefetchLocalidadesBuscador no lanza aunque el catálogo falle", async () => {
    const { geo, cache, api } = await servicioFresco();
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    cache.getCatalogoLocalidades.mockRejectedValue(new Error("idb roto"));
    api.get.mockRejectedValue(new Error("catalogo caido"));

    expect(() => geo.prefetchLocalidadesBuscador()).not.toThrow();
    await new Promise((r) => setTimeout(r, 5)); // deja correr el idle

    warnSpy.mockRestore();
  });
  it("descarta un build en vuelo si el índice se invalida mientras construye", async () => {
    const { geo, cache } = await servicioFresco();

    // Catálogo "lento": controlamos cuándo resuelve.
    let resolverCatalogo;
    cache.getCatalogoLocalidades.mockReturnValue(
      new Promise((resolve) => {
        resolverCatalogo = resolve;
      }),
    );

    const enVuelo = geo.getLocalidadesBuscador(); // arranca y queda esperando
    geo.invalidarBuscadorLocalidades(); // llega un cambio de catálogo

    resolverCatalogo(LOCS);
    await expect(enVuelo).resolves.toBeNull(); // el build viejo se descarta

    // El próximo intento reconstruye con datos frescos
    cache.getCatalogoLocalidades.mockResolvedValue(LOCS);
    await expect(geo.getLocalidadesBuscador()).resolves.toBeTruthy();
  });
});
