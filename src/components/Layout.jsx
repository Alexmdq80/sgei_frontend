  ...
  const renderNavItem = (item, depth = 0) => {
    if (item.isSubgroup) {
      const hasActiveChild = item.items.some((child) =>
        child.isSubgroup
          ? child.items.some((gc) => location.pathname === gc.path)
      ...
      return (
        <li key={item.name} className="flex flex-col">
          ...
          >
            ...
            <ChevronDown
              className={`w-3 h-3 transition-transform duration-200 ${item.isOpen ? "rotate-180" : ""}`}
            />
          </button>
          {item.isOpen && (
            <ul
              className={`mt-1 ml-4 space-y-1 ${depth === 0 ? "border-l border-secondary-800" : ""}`}
            >
    ...
    <div className="min-h-screen bg-secondary-50 flex font-sans">
      ...
      {notification && (
        <div className="fixed top-20 right-8 z-[200] max-w-sm w-full animate-fadeInRight">
          <div
            className={`p-4 rounded-2xl shadow-2xl border-l-4 flex items-start gap-4 ${
          ...
          >
            <div
              className={`p-2 rounded-xl shrink-0 ${notification.type === "success" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
            >
              {notification.type === "success" ? (
      ...
      {/* Sidebar */}
      <aside
        className={`${isSidebarOpen ? "w-64" : "w-20"} bg-secondary-900 text-white transition-all duration-300 flex flex-col shadow-xl`}
      >
        <div className="p-6 flex items-center justify-between border-b border-secondary-800">
          {isSidebarOpen && (
            <span className="text-xl font-bold text-primary-400">SGEI</span>
          )}
          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="p-1 hover:bg-secondary-800 rounded"
          >
        ...
        <div className="p-4 border-t border-secondary-800">
          {isSidebarOpen && (
            <div className="mb-4 px-3 py-2 bg-secondary-800/50 rounded-xl border border-secondary-700/50">
              <p className="text-[9px] font-black text-secondary-500 uppercase tracking-widest">
                Actuando como
              </p>
              <p className="text-xs font-bold text-primary-400 truncate mt-1">
                {actingRole.name}
              </p>
              {actingRole.context && (
                <p className="text-[10px] font-medium text-secondary-300 mt-0.5 truncate">
                  {actingRole.context}
                </p>
              )}
            </div>
        ...
        <header className="h-16 bg-white border-b border-secondary-200 flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center gap-4 relative" ref={userMenuRef}>
            <button
              ...
              className="flex items-center gap-3 p-1.5 hover:bg-secondary-50 rounded-xl border border-transparent group transition-all"
            >
                ...
                <div className="flex items-center justify-end gap-1.5 mt-0.5">
                  <span
                    className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter border ${
                      actingRole.type === "admin"
                        ? "bg-rose-50 text-rose-600 border-rose-100"
                        : actingRole.type === "institutional"
                  ...
                  {actingRole.context && (
                    <p className="text-[9px] text-secondary-400 font-bold uppercase truncate max-w-[120px]">
                      {actingRole.context}
                    </p>
                  )}
                </div>
            ...
            {isUserMenuOpen && (
              <div className="absolute top-full right-0 mt-2 w-64 bg-white border border-secondary-200 rounded-2xl shadow-2xl py-2 z-50">
                ...
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-2.5 text-sm text-secondary-700 hover:bg-primary-50 transition-colors"
                >
                  <User className="w-4 h-4" />
                  Mi Perfil
                </Link>
                {(user?.roles?.length || 0) +
                  (user?.escuelas_personas?.filter((l) => l.verified_at)
                    .length || 0) >
                  1 && (
                  <Link
                    to="/select-role"
